...
if __name__ == '__main__':
    main()
[donald@fear ~]$ cp /mnt/dloads/sandbag.py .
[donald@fear ~]$ ls
 ai1.py        Desktop     Downloads   games   Music     nbd4.py    ndb3.py   '#PASSWORDS#'   PASSWORDS~   Playlists   sandbag.py   Videos
 configstuff   Documents   emacs.d     Games   nbd2.py   nbd4.py~   ndb3.py~   PASSWORDS      Pictures     Public      Templates
[donald@fear ~]$ cat sandbag.py 
#!/usr/bin/env python3

import aiofiles, asyncio, argparse, ast
...
async def run(args):
    rate = ast.literal_eval(args.rate) # if user inputs bad expression let it die here
    async with aiofiles.open(args.path, 'rb') as file:
        server = sandbag(file, args.size, args.gate, (*rate, (2**64, 1)))
        async with asyncio.TaskGroup() as tg:
            tg.create_task(server.start_server(host=args.host, port=args.port))
...
def main():
    ...
    parser.add_argument('--size', default=512,   type=int,   help='block size for device')
    parser.add_argument('--wait', default=0.1,   type=float, help='delay at end of each inner loop')
    asyncio.run(run(parser.parse_args()), debug=True)
